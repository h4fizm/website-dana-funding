  ...
  static editUser(req, res) {
    ...
    User.findByPk(userId)
      .then((user) => {
        ...
        res.render("admin/edituser", { user }); // Render halaman edit dengan data user
      })
      .catch((err) => {
        console.error(err);
  ...
  static updateUser(req, res) {
    ...
    if (password) {
      bcrypt.hash(password, 10, (err, hashedPassword) => {
        ...
        updatedData.password = hashedPassword; // Encrypt password sebelum disimpan
        // Update user di database
        User.update(updatedData, {
          where: { id: userId },
        })
          .then(() => {
            res.redirect("/admin/users"); // Redirect ke halaman daftar user setelah update
          })
          .catch((err) => {
            console.error(err);
